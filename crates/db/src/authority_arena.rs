...
impl AuthorityArena {
    ...
    pub async fn revoke_by_stratum(
        ...
        let drain_seq = events::current_seq(&self.pool, &self.session_id).await?;
        let revoked_ids =
            tokens::revoke_by_stratum(&self.pool, &self.session_id, self.epoch, stratum_threshold)
                .await?;
        if let Err(e) = crate::descriptors::delete_for_caps(&self.pool, &revoked_ids).await {
    ...
    pub async fn revoke_epoch(
        ...
        let drain_seq = events::current_seq(&self.pool, &self.session_id).await?;
        let revoked_ids = tokens::revoke_epoch(&self.pool, &self.session_id, self.epoch).await?;
        if let Err(e) = crate::descriptors::delete_for_caps(&self.pool, &revoked_ids).await {
    ...
    pub async fn transfer_root(
        ...
        let old_root_id =
            match tokens::find_root_cap_in_tx(&mut tx, &self.session_id, old_actor_id).await? {
                Some(id) => id,
                None => {
...
impl Authority {
    ...
    pub async fn delegate(
        ...
        if !self.permissions.is_empty() {
            if let Some(bad) = reduced_perms.iter().find(|p| !self.permissions.contains(p)) {
        ...
        let unknown =
            crate::methods::unknown_addresses(&self.pool, &self.session_id, reduced_perms).await?;
        if !unknown.is_empty() {
            return Err(DbError::Conflict(format!(
