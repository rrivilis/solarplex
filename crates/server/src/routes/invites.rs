...
#[autometrics]
async fn preview(Path(id): Path<String>, State(state): State<Arc<AppState>>) -> impl IntoResponse {
    ...
    let session_name = sessions::get(&state.db, &row.session_id)
        .await
        .map(|s| s.name)
    ...
    let inviter_name = actors::get(&state.db, &row.invited_by)
        .await
        .map(|a| a.name)
...
async fn redeem(
    ...
    if let Some(res) = crate::rate_limit::gate_global(
        crate::rate_limit::GlobalRateLimitKey::InviteRedeemAttempt {
    ...
    let actor_email = actors::get(&state.db, &actor_id)
        .await
        .ok()
        .and_then(|a| a.email);
    ...
    let previous_owner = if invite.role == "owner" {
        sessions::list_memberships(&state.db, &invite.session_id)
            .await
            .ok()
    ...
    let membership = match sessions::add_member(
        ...
        &actor_id,
        &invite.role,
        invite.escalation_order,
        invite.escalation_timeout,
    )
    .await
    {
        Ok(m) => m,
    ...
    if let Some(grant) = invites::parse_cap_grant(&invite) {
        let observed_seq = events::current_seq(&state.db, &invite.session_id)
            .await
            .unwrap_or(0);
        ...
        .await
        {
            Ok(row) => {
                if let Err(e) =
                    db::descriptors::grant(&state.db, &actor_id, &format!("cap/{}", row.id)).await
                {
...
async fn revoke(
    ...
    match invites::revoke(&state.db, &id).await {
        ...
        Err(db::DbError::NotFound) => (
            ...
            "invite not found, already redeemed, or already revoked",
        )
            .into_response(),
