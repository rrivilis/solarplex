...
use protocol::messages::{
    AgentStatusPayload, ArtifactPayload, ContextEntryAddedPayload, MessagePostedPayload,
    OwnershipTransferredPayload, SessionRenamedPayload, SessionStatusPayload,
    ShellCommandCompletedPayload, ShellCommandStartedPayload, WsMessage, WsPayload,
};
...
pub fn router() -> Router<Arc<AppState>> {
    Router::new()
        ...
        .route(
            "/{id}/approvals",
            get(list_approvals).post(create_approval_rest),
        )
...
async fn create_session(
    ...
    if let Some(res) = crate::rate_limit::gate_global(
        crate::rate_limit::GlobalRateLimitKey::SessionCreate {
...
async fn list_sessions(
    ...
    match sessions::list_by_actor(&state.db, &actor_id).await {
        Ok(rows) => {
            ...
            let names = actors::get_many(&state.db, &creator_ids)
                .await
                .unwrap_or_default();
...
async fn get_session(
    ...
    if let Err(res) =
        crate::auth::require_session_member(&state.db, &headers, &id, MemberRole::Observer).await
    {
        return res;
    }
    match sessions::get(&state.db, &id).await {
        Ok(session) => {
            ...
            let names = actors::get_many(&state.db, &actor_ids)
                .await
                .unwrap_or_default();
            let enriched_members: Vec<serde_json::Value> = members
                .iter()
                .map(|m| {
...
async fn update_session(
    ...
    let actor = match crate::auth::require_session_member(
        &state.db,
        &headers,
        &id,
        MemberRole::Collaborator,
    ...
    if let Some(ref new_name) = body.name {
        ...
        let old_name = sessions::get(&state.db, &id)
            .await
            .ok()
...
async fn add_member(
    ...
    // policy.
    if let Err(res) =
        crate::auth::require_session_member(&state.db, &headers, &id, MemberRole::Owner).await
    {
        return res;
    }
    ...
    match sessions::add_member(
        ...
        &body.actor_id,
        &body.role,
        body.escalation_order,
        body.escalation_timeout,
    )
    .await
    {
        Ok(m) => {
            ...
            let joiner_name = actors::get(&state.db, &body.actor_id)
                .await
                .ok()
                .map(|a| a.name);
            emit_to_session(
...
async fn create_invite(
    ...
    let target_role: protocol::types::MemberRole = match body.role.parse() {
        ...
        _ => {
            return (
                ...
                "role must be owner, collaborator, or observer",
            )
                .into_response()
        }
    };
    let cap = match (body.cap_permissions, body.cap_ttl_secs) {
        (Some(permissions), Some(ttl_secs)) => Some(invites::CapGrant {
            permissions,
            ttl_secs,
        }),
        (None, None) => None,
    ...
    let caller = match sessions::require_membership(
        ...
        &id,
        &invited_by,
        protocol::types::MemberRole::Observer,
    )
    .await
    {
        Ok(m) => m,
            ...
            let mailbox_status = match &row.invitee_email {
                ...
                Some(email) => match actors::find_by_email(&state.db, email).await {
                    Ok(Some(mailbox_actor_id)) => {
                        ...
                        if let Err(e) =
                            db::mailbox::insert_route(&state.db, &mailbox_actor_id, &uri).await
                        {
...
async fn transfer_ownership(
    ...
    let from = match crate::auth::require_session_member(
        &state.db,
        &headers,
        &id,
        MemberRole::Owner,
...
async fn issue_attach_token(
    ...
    let issuer_id = match crate::auth::require_session_member(
        &state.db,
        &headers,
        &id,
        MemberRole::Collaborator,
    ...
    if let Err(e) =
        sessions::add_member(&state.db, &id, &body.actor_id, &body.role, None, None).await
    {
    ...
    .await
    {
        Ok(row) => {
            if let Err(e) =
                db::descriptors::grant(&state.db, &row.actor_id, &format!("cap/{}", row.id)).await
            {
...
async fn list_events(
    ...
    let role = sessions::get_membership(&state.db, &id, &actor_id)
        .await
    ...
    match db::events::list(&state.db, &id, q.after_seq, q.limit.unwrap_or(100)).await {
        Ok(mut events) => {
            events.retain_mut(
                |e| match crate::event_visibility::min_role_for_type(&e.r#type) {
                    None => true,
...
async fn import_artifact(
    ...
    let source_name = source_session
        .map(|s| s.name)
        .unwrap_or_else(|_| body.source_session_id.clone());
    ...
    let target_artifacts = db::artifacts::list(&state.db, &target_id)
        .await
        .unwrap_or_default();
...
async fn agent_heartbeat(
    ...
    let cell = state
        .inflight_heartbeat_checks
        .entry(body.cap_id.clone())
        .or_insert_with(|| Arc::new(tokio::sync::OnceCell::new()))
        .clone();
    let result = cell
        .get_or_init(|| crate::auth::require_cap_auth_typed(&state.db, &id, &body.cap_id))
        .await
        .clone();
...
async fn register_methods(
    ...
    match db::methods::register_bulk(&state.db, &id, &slug, &body.methods).await {
        Ok(n) => (StatusCode::OK, Json(serde_json::json!({ "registered": n }))).into_response(),
        Err(e) => (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()).into_response(),
    }
}
