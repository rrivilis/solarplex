...
async fn scan_result(
    ...
    if let Some(tlsh_str) = &body.tlsh {
        ...
        match closest {
            Some((dist, family_id)) if dist < db::artifact_reputation::TLSH_CLUSTER_THRESHOLD => {
                let _ = db::artifact_reputation::assign_family(
                    &state.db,
                    &body.sha256,
                    &family_id,
            ...
            _ => {
                ...
                match db::artifact_reputation::create_tlsh_family(&state.db, &body.sha256, tlsh_str)
                    .await
                {
                    Ok(family_id) => {
                        let _ = db::artifact_reputation::assign_family(
