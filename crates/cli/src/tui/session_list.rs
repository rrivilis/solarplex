...
impl SessionList {
    pub fn new(rows: Vec<SessionRow>) -> Self {
        Self { props: Props::default(), rows, selected: 0, last_area: Rect::default() }
    }
    ...
    fn on(&mut self, ev: &Event<NoUserEvent>) -> Option<Msg> {
        if let Some(key) = ev.as_keyboard() {
            return match key {
                ...
                KeyEvent { code: Key::Enter, modifiers: KeyModifiers::NONE } => {
                    self.selected_id().map(|id| Msg::EnterSession(id.to_string()))
                }
                KeyEvent { code: Key::Char(':'), modifiers: KeyModifiers::NONE } => Some(Msg::OpenCommandLine),
                KeyEvent { code: Key::Esc, modifiers: KeyModifiers::NONE } => Some(Msg::AppClose),
                _ => None,
            };
        }
        ...
        if let Some(mouse) = ev.as_mouse() {
            if matches!(mouse.kind, MouseEventKind::Down(MouseButton::Left)) {
                if let Some(idx) = self.row_at(mouse) {
                    self.selected = idx;
                    return self.selected_id().map(|id| Msg::EnterSession(id.to_string()));
                }
            }
        }
