...
pub async fn cmd_why(actor: String, entity: Option<String>, ctx: &Ctx) -> Result<()> {
    ...
    let data = client
        .auth_why(session_id, actor_id, entity.as_deref())
        .await?;
    ...
    if let Some(m) = data.get("membership") {
        ...
        println!(
            "  approve  {}",
            yesno(m["can_approve"].as_bool().unwrap_or(false))
        );
        println!(
            "  write    {}",
            yesno(m["can_write"].as_bool().unwrap_or(false))
        );
        println!();
    } else {
        println!(
            "{} actor/{actor_id} is not a formal session member",
            yellow("⚠")
        );
        println!();
    }
    ...
    for cap in caps {
        ...
        if lineage.len() > 1 {
            ...
            for (i, hop) in lineage.iter().enumerate() {
                ...
                let root_mark = if hop["is_root"].as_bool().unwrap_or(false) {
                    " [root]"
                } else {
                    ""
                };
                let prefix = if i + 1 == lineage.len() {
                    "└─"
                } else {
                    "├─"
                };
...
pub async fn cmd_lineage(cap: String, ctx: &Ctx) -> Result<()> {
    ...
    for hop in chain {
        ...
        let tree_prefix = if is_root {
            ...
            format!("  {}  ", "│ ".repeat((hop_idx as usize).saturating_sub(1)))
        };
        ...
        println!("{}    actor/{actor_id} ({actor_name})", tree_prefix,);
        if !is_leaf {
            println!("{tree_prefix}│");
        }
    }
...
fn require_session(ctx: &Ctx) -> Result<&str> {
    ctx.session_id.as_deref().ok_or_else(|| {
        anyhow::anyhow!("--session / SOLARPLEX_SESSION_ID required for this command")
    })
}
